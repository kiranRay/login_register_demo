...
module.exports = function (req, res, next) {
    ...
    pool.query(`select email from public.register`,(err,result)=>{
        ...
        if(u_mail){
            return  res.render('invite',{mail: req.session.user.email,msg: 'This Email is already registered!!!'}); 
        }
        else{
            pool.query(`select child from public.invited`,(err,result)=>{
                ...
                if(u_mail){
                    return  res.render('invite',{mail: req.session.user.email,msg: 'This Email is already invited!!!'}); 
                }
                else{
                    next();
                } 
